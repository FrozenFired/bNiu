...
exports.newFiles = async(req, res, next) => {
	...
	try {
		...
		if(dataFls instanceof Array) {
			for(const i=0;  i<dataFls.length; i++) {
				const fl = dataFls[i];
				if(fl && fl.originalFilename) {
					const file = await recuPics(fl, flName, flDir);
					files.push(file);
				}
			}
		} else {
